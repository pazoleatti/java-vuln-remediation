...
- `--vuln=<vulnerabilityHash>` — optional. Enables **targeted mode**: the run processes exactly one finding identified by its `vulnerabilityHash`. Bind to `targetVulnHash` and forward it to `init_run` — the MCP filters to that single occurrence and aborts the run via `targetFound: false` if the hash is not present in the report. `--limit` is implicitly ignored in targeted mode. In targeted mode `init_run` also forces the notexploit policy to "include" so the target is always seeded as `pending` even if its SAST `decision.type == "notexploit"`.
...
10. **Notexploit policy is enforced inside `init_run`, not by you.** Forward `{ commit, includeNotexploit, targetVulnHash? }` to `init_run` — the MCP reads the SAST report straight from the per-commit cache, performs the catalog × occurrences join, and applies the policy. When `includeNotexploit` is false, findings with `decision.type == "notexploit"` are seeded as `skipped_notexploit` and the original SAST decision is persisted on the record for Phase 4. You do **not** build any payload from the report and you do **not** override the operator's `--include-notexploit` flag — the operator's invocation is authoritative. The single exception is targeted mode (constraint 11).
11. **Targeted mode (`--vuln`) overrides notexploit filtering and `--limit`.** When `--vuln=<hash>` is set, pass `targetVulnHash: <hash>` to `init_run`. The MCP narrows to that single occurrence and forces `includeNotexploit = true` internally, so the target always reaches `pending` regardless of the operator's flag. If the response carries `targetFound: false`, abort the run with a clear message before creating the branch or building the index — `init_run` will have seeded nothing.
...
4. `init_run({ commit: <commit>, includeNotexploit: <flag>, targetVulnHash: <hash>|undefined })` — **one** call. State-mcp reads the same per-commit cache that `get_report` populated, performs the `vulnerabilitiesInfo × resultInfo[].vulnerabilities[]` join, normalises `decision`, applies the notexploit policy, and seeds the state file. Capture `sastUuid` (== `report.taskUuid`) and `skippedNotexploit` from the response for the Phase 4 header (always `0` in targeted mode). If `targetVulnHash` is set and the response carries `targetFound: false`, **abort the run** before creating the branch or building the index — nothing was seeded.
...
4. For each `vulnerabilityHash` in order: invoke `triage-agent` with `{ reportUuid, vulnerabilityHash: <hash> }`. The delegate writes its verdict via `update_triage_result` and exits.
