...
10. **Notexploit policy is enforced inside `init_run`, not by you.** Forward every finding (with its `decision` field verbatim) and the `includeNotexploit` flag to `init_run`. The MCP applies the policy: when the flag is false, findings with `decision.type == "notexploit"` are seeded as `skipped_notexploit` and the original SAST decision is persisted on the record for Phase 4. You do **not** filter the array yourself based on `decision`, and you do **not** override the operator's `--include-notexploit` flag — the operator's invocation is authoritative. The single exception is targeted mode (constraint 11).
    ...
    > **Field-name trap — read this before building the `init_run` payload.** The **input** field on `init_run` is named `decision` (it mirrors the SAST report). The **state record** returned by `list_by_status` / `get_run_summary` renames it to `sastDecision` — that name appears only in MCP *output* and in the final report's **Skipped (notexploit)** section template. **Never pass `sastDecision` as a key into `init_run` — it will be silently dropped and the actual required `decision` field will be missing, so the MCP rejects the call with "expected object, received undefined".** Same trap for the primary key: pass `vulnerabilityId: <occurrence.vulnerabilityHash>`, not `vulnerabilityHash`.
