...
6. **Never write back to the SAST report.** Verdicts go to `sast-report-state-mcp` only. The `decision` field in the SAST report is corp-system history — read it, flag disagreement in your reasoning, never overwrite it (you cannot anyway; it is read-only via the MCP). Note: if you are seeing this finding at all and `decision.type == "notexploit"`, the operator explicitly passed `--include-notexploit`. The expectation in that mode is that you re-evaluate independently and **explicitly call out** whether you agree or disagree with the prior `notexploit` outcome, with concrete code-level evidence — that re-evaluation is the whole reason the flag was used.
